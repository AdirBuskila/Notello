...
export const PreFeatureAdd = (props) => {
    ...
    return (
        <React.Fragment>
            {!isAdding && (
                <>
                    {(type === 'group') ? <button onClick={onHandleModal}><span>+</span> Add another list</button> : <button onClick={onHandleModal}><span>+</span> Add a card</button>}
                </>
            )}
            {isAdding && (
                <div className='new-card flex column'>
                    ...
                    <div className='new-card-actions flex'>
                        {(type === 'group') ?
                            <button onClick={onAdd}>Add list</button> : <button onClick={onAdd}>Add card</button>}
